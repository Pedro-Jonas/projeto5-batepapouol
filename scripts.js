...
function escrevermensagens(mensagens){
    ...
    for (let i= 0; i < (todasasmensagens.length-1); i++) {
        ...
        if (element.type === "private_message" && element.to === nome){
            localmensagens.innerHTML += `<div class="caixa reservado"> <p>(${element.time})</p> ${element.from} <h1>para</h1> ${element.to} <h1>${element.text}</h1></div>`
        }
        atualizarmensagens()
    }
}
